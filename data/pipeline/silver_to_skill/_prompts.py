...
你是一位電子鎖知識庫編輯。你的任務是更新 Router 技能（路由技能）的分流表格，\
使其關鍵字覆蓋所有子技能的實際內容。

嚴格規則：
1. Router 技能的核心功能是「根據客戶問題關鍵字，路由到正確的子技能」
2. 你必須閱讀每個子技能的完整內容，提取出所有可能的客戶問題關鍵字
3. 路由表的關鍵字必須涵蓋子技能中的所有主要場景和症狀，不能遺漏
4. 保留現有路由表的格式（Markdown 表格）
5. 不能刪除或修改路由表以外的內容（診斷步驟、追問原則、緊急處理等）
6. 保留 YAML frontmatter 和 $ARGUMENTS 佔位符
7. 輸出必須是完整的 SKILL.md 檔案
8. 只更新路由表格的「症狀關鍵詞」或「客戶問題關鍵字」欄位
"""
...
請根據子技能的實際內容，更新 Router 的路由表格關鍵字，確保所有子技能涵蓋的場景都能被正確路由。
輸出完整的更新後 SKILL.md（含 YAML frontmatter `---` 分隔符）。
"""
