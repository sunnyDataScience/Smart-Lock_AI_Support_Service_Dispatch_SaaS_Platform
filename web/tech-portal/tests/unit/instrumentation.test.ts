...
describe("instrumentation register()（OTLP opt-in）", () => {
  ...
  it("env 設定 → registerOTel 以預設 serviceName=tech-portal 呼叫", async () => {
    ...
    expect(cfg.serviceName).toBe("tech-portal");
  ...
  it("OTEL_SERVICE_NAME 設定 → 覆寫 serviceName（與 api 同語意）", async () => {
    vi.stubEnv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4318");
    vi.stubEnv("OTEL_SERVICE_NAME", "tech-portal-uat");
    const register = await loadRegister();

    await register();

    expect(registerOTelMock.mock.calls[0][0].serviceName).toBe("tech-portal-uat");
  });
