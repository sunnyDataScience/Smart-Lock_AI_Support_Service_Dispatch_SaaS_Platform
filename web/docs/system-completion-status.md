...
| Flow 4 缺料 | **90%** | **2026-06-05 admin 補料管理彙整 UI 落地**：service `list_pending_material_requests`（跨工單 SELECT JOIN，urgency 排序）+ endpoint `GET /tenants/{tenantId}/material-requests` (listPendingMaterialRequestsV2) + Next.js page `/admin/material-requests`（table view + urgency filter chips + WO 鏈結 + i18n zh-TW/en + Sidebar nav）。**剩餘 10% 缺口**：`supply_arrived` 完結 event_type + admin 標記補料完成 endpoint（OOSCope 本輪，新一輪 follow-up） |
