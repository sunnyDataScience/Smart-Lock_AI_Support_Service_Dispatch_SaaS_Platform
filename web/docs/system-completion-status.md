...
| Flow 8 二次派工 | **100%** | **2026-06-05 兩輪 e2e 完成**：(1) prior commit `d365b049` backend 落地（`_REASSIGN_FROM={assigned,accepted,in_progress}` + `reassign_order` service NO_OP_SAME_TECHNICIAN guard + dispatch_logs/work_order_events 雙表 audit + WS publish work_order.reassigned + endpoint `:reassign`）/ (2) 本輪前端整合 — work-orders/[id]/page.tsx 加 `REASSIGN_FROM` const (accepted/in_progress) + `canAssign` derived 擴展涵蓋 ASSIGN_FROM ∪ REASSIGN_FROM + `handleAssign` 內以 status 判斷走 `:reassign` (body `{technician_id, reason}` 必填，預設 fallback「強制改派（admin override）」) 或原 `:assign` (body `{technician_id, reason_code, reason_text?}`)；既有 AssignDialog `titleReassign` UI 自動 reused |
