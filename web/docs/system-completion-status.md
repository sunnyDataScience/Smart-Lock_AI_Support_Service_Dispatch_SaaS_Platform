...
| Flow 2 拒單重派 | **100%** | — |
| Flow 3 範圍變更 | **85%** | **2026-06-05 BUILD — proposal INSERT 鏈路 + token mint 落地（gap #1+#2 解，#3 留 CR-0017）**：deep audit 三 critical gap 中本輪解決 (1) **proposal 建立路徑** — `record_scope_change` 改寫：除 `work_order_events 'scope_change'` 仍寫，新增 `INSERT INTO scope_changes (work_order_id, technician_id, reason, original_scope, new_scope, original_price, new_price, status='pending')` 路徑落地（取 wo.technician_id + wo.estimated_price 作 NOT NULL 必填欄位；422 fallback 防呆若 technician_id 為 NULL）；event payload 補 `scope_change_id` 反向指標 + actor_user_id 鏈路。(2) **token mint** — 新增 `public_token.generate_token(scope_change_id, purpose='scope_change', ttl_days=7, tenant_id)` caller，mint 結果僅 log（暫不暴露至 API response，避免破壞 WorkOrderEnvelope response_model；等 CR-0017 LINE Flex push 重建時決定 surface 機制）。剩 (3) **LINE Flex 主動通知 + WS publish** — 與 Flow 11 同病鏈路，待 [`docs/_audit/flow-11-agent-rewrite-impact.md`](../../docs/_audit/flow-11-agent-rewrite-impact.md) → CR-0017 開立並裁決。**剩 15% 缺口**：LINE Flex 主動 push 客戶（CR-0017 待開）。|
