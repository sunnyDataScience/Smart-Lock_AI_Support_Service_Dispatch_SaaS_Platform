...
| Flow 8 二次派工 | **85%** | **2026-06-05 deep audit + BUILD**：deep audit 揪出 70% 略高 — `_ASSIGN_FROM={created,assigned}` 只支援 assigned 階段換手，accepted/in_progress 無法 reassign（必須 cancel-rebuild 破壞性流程）；dispatch_log enum 有 `reassign` 但 0 caller。**本輪 BUILD**：(1) 新 `_REASSIGN_FROM={assigned,accepted,in_progress}` / (2) service `reassign_order(*, tenant_id, wo_id, new_technician_id, reason, actor_user_id)`：驗 _REASSIGN_FROM + 防 NO_OP_SAME_TECHNICIAN 422 + new_tech tenant/active 驗證 + UPDATE technician_id force-back to 'assigned' + INSERT dispatch_logs action='reassign' audit + INSERT work_order_events event_type='reassign' subflow timeline + WS publish work_order.reassigned / (3) endpoint `POST /tenants/{tid}/work-orders/{id}:reassign` (reassignWorkOrderV2, `_DISPATCH_ALLOWED_ROLES` + bypass audit for customer_service)。**剩 15% 缺口**：admin work-order detail page 「強制改派」按鈕 + 技師選擇器（OOSCope 本輪，前端 follow-up） |
