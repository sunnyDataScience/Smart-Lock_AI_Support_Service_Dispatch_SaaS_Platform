...
**最後更新：** 2026-07-08（**web 檔案層拆分 monorepo（0707 AI #2 完整版）**，branch `refactor/web-apps-split` — 業主裁決檔案層真拆。web/ 轉 npm workspaces：apps/{dispatch,tech,landing,platform} + packages/shared（@smartlock/shared）；183 檔 import 改寫 @shared/*；Dockerfile 參數化 ARG APP 四站共用；四 compose + deploy/web.sh + 3 個 CI workflow 路徑同步；ADR-028 立案（supersede ADR-023）+ CIA CR-0122。驗證：四 app tsc 0 + next build 全綠（tech 產物僅 16 條路由，拆分實證）；Docker image ×4 + 各站 smoke 於 merge 前補驗。）
