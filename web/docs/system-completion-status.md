...
**最後更新：** 2026-07-06（**廠商審核移植到 platform console（:3003）+ 品牌頁移除**，branch `feat/platform-vendor-approvals` — 業主指示把品牌 `admin/vendor-approvals` 移到 `:3003/platform`（獨立「廠商審核」頁）。vendors＝發案方登入帳號（role=vendor），與 brand_applications（意向書）是不同生命週期階段。**後端**：新 `platform_vendor_service`（cross-tenant list + 委派 vendor_service 狀態機）+ `platform_vendors.py` 3 端點（GET/:approve/:reject，require_platform_admin）；品牌 vendors_v2 刪 approve/reject 寫端點（留 GET 唯讀）。vendors.approved_by 有 FK→品牌 users，平台管理員非品牌 user → 平台核准存 approved_by=NULL。**前端**：新 `/platform/vendors`（狀態篩選+核准/拒絕）；nav 加「廠商審核」。**品牌移除**：刪頁+側欄項+Store icon+rolePolicy 死條目+i18n（parity 2654）；/vendors/register 保留。驗證：管理測試 5 項+全套 **1623 passed** 零回歸、tsc 0。**已部署本機 docker（platform api+web、dispatch web）**。）
