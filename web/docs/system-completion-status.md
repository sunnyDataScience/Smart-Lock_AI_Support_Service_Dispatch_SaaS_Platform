...
**最後更新：** 2026-06-28（**CR-0108 S3 LINE 進線自動帶 Case — M01 收尾**，branch `feat/cr-0108-s3-line-case` — 原 LINE 進線直接建 problem_card 不掛 Case，與手動渠道資料模型不一致。`problem_card_service` 新 best-effort `_ensure_line_case`：escalation 建/併草擬卡時 ensure 一張 `source_channel=line` 的 intake_case 並連 `problem_cards.case_id`（一卡一 Case、已連則略過、客戶資訊取對話 user 沿 D6、失敗不阻斷建卡）。**LINE 進線至此也收斂到 Case 模型，M01（CR-0108）後端+前端+LINE 三段全到位**。API test_cr_0108_s3_line_case 2/2 + **全套 1461 passed 無回歸**。**待部署 api**）
