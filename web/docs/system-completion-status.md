...
**最後更新：** 2026-06-04（Flow 3 範圍變更取證 — 後端 v2 + 前端 admin/consumer + scope_change_service 全綠，80% → 90%；缺口從 vague「客戶核准流程簡化」精化為兩個具體 gap：LINE Flex 主動通知客戶 + WS publish）
**對應分支：** `docs/wbs-flow-3-取證`（基於 dev_new_arch HEAD `4ff6373c`）
