...
**最後更新：** 2026-07-05（**師傅登入頁移除品牌入口連結與版本 footer（web）**，branch `fix/tech-login-remove-brand-link-footer` — 業主要求 `/tech-login` 移除底部「我是品牌 / 經銷 / 鎖店 →」跨站連結與「v0.1 — 技師端 PWA」版本字。移除兩個 JSX 區塊（i18n 鍵保留、無其他使用點）；底部收在「申請成為師傅」連結。tsc 0 + 重建 tech-web+dispatch web + Playwright 實測（3001/tech-login 兩者皆消失、申請/忘記密碼保留）。純前端刪元素 CIA 豁免。**已部署本機 docker web（tech+dispatch）**。）
