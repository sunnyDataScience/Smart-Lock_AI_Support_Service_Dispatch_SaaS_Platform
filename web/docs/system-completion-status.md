...
**最後更新：** 2026-07-07（**對話照片泡泡隱藏「[照片]」佔位文字**，branch `fix/conversations-photo-placeholder`，CR-0119 收尾 — 業主回饋照片已顯示時佔位文字冗餘。ChatTimeline：`media_url` 存在且 content 恰為「[照片]」→ 只顯示照片;無照片的歷史訊息/有實際文字者照常。純前端、CIA 豁免。驗證:tsc 0、重建 dispatch web、Playwright 端到端(泡泡內僅照片無文字,測試資料已清)。**已部署本機 docker(dispatch web)**。）
