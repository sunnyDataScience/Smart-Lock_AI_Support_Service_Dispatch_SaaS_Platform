...
| Flow 2 拒單重派 | **100%** | — |
| Flow 3 範圍變更 | **90%** | **2026-06-05 兩輪推進**：(1) prior commit `239dff9c` 補 proposal INSERT 鏈路 + token mint（解 deep audit gap #1+#2，65% → 85%）/ (2) 本輪 `admin_override` 補完 — 新 service `scope_change_service.admin_override(*, tenant_id, proposal_id, approved_by_user_id, reason)` 處理「客戶不回應 / 超時 / 業務裁決」場景：JOIN wo 三段 tenant 驗證 (403 CROSS_TENANT_WRITE) + 404/409 guards + CAS UPDATE `status='admin_override'`, `customer_decision='continue'`, `approved_by=user.id` + 連動 wo.status → in_progress + audit log；新 endpoint `POST /tenants/{tid}/scope-changes/{proposalId}:admin-override` (operation_id `adminOverrideScopeChangeV2`，`_DISPATCH_ALLOWED_ROLES` + idempotency)。**注意**：earlier audit 誤判 consumer 流不能用 — 取證重核 `respondScopeChangeV2` (`consumer_v2.py:170`) + service `respond_public` 早已存在且正確，是「沒 row 可回應」非「endpoint 缺」。**剩 10% 缺口**：LINE Flex 主動 push 客戶（CR-0017 待開，與 Flow 11 同病）|
