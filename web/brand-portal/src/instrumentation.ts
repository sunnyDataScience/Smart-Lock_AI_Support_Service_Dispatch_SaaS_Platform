 ...
 *
 * 範式比照 api/core/observability.py（brand-portal 為四站參考實作，其餘三站後續複製）：
 ...
 *   洩漏面不存在，風險低。@vercel/otel 2.x 無出站屬性過濾 hook
 *   （`attributesFromHeaders` 僅能「新增」header 衍生屬性，非過濾），深度遮蔽
 *   （比照 api `_PIIScrubExporter` 包裝 exporter 過 scrub）需自訂 SpanProcessor
 *   耦合 OTel SDK 內部行為——留待三站複製輪一併補上。
 *
