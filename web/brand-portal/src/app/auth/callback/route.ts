...
export async function GET(request: NextRequest) {
  ...
  if (!code) {
    return NextResponse.redirect(new URL("/login?sso_error=missing_code", request.url));
  }
  ...
  try {
    ...
    return NextResponse.redirect(new URL("/login?sso_error=idp_unreachable", request.url));
  }
  const data = await tokenResp.json().catch(() => ({}));
  const accessToken: string | undefined = data.access_token;
  const refreshToken: string | undefined = data.refresh_token;
  if (!tokenResp.ok || !accessToken) {
    return NextResponse.redirect(new URL("/login?sso_error=exchange_failed", request.url));
  }
