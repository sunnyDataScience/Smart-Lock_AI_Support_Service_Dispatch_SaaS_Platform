...
describe("instrumentation register()（OTLP opt-in）", () => {
  ...
  it("env 設定 → registerOTel 以預設 serviceName=brand-portal 呼叫", async () => {
    ...
    expect(cfg.serviceName).toBe("brand-portal");
  ...
  it("OTEL_SERVICE_NAME 設定 → 覆寫 serviceName（與 api 同語意）", async () => {
    vi.stubEnv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://localhost:4318");
    vi.stubEnv("OTEL_SERVICE_NAME", "brand-portal-uat");
    const register = await loadRegister();

    await register();

    expect(registerOTelMock.mock.calls[0][0].serviceName).toBe("brand-portal-uat");
  });
