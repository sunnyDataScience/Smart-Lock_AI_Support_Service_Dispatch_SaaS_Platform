...
def _onboarding_guide(app: dict) -> str:
    ...
    return (
        ...
        f"      docker compose -f docker-compose.dispatch.yml up -d --build\n"
        f"    BRAND={slug} docker compose -f docker-compose.dispatch.yml --profile init run --rm db-init\n"
        f"\n"
        f"[3] 接技師共用庫(CR-0112 方法 B 雙庫;不設則單庫 fallback):\n"
        f"    TECH_POSTGRES_URI=postgresql://lock:0000@tech-db:5432/lock_tech \\\n"
        f"      BRAND={slug} docker compose -f docker-compose.dispatch.yml up -d api\n"
        f"\n"
