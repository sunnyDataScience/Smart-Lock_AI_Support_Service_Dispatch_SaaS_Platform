...
CR-0112 方案 B（技師身分庫物理拆分）：本模組支援兩條連線 ——
  - 主連線（POSTGRES_URI）：品牌營運庫（工單/派工/帳務/後台帳號）。
  - 技師庫連線（TECH_POSTGRES_URI）：技師身分域（users 技師列/technicians/
    排班/品牌授權/認證/技能/lifecycle 事件）。
**Fallback 安全閥**：TECH_POSTGRES_URI 未設定時，get_tech_conn() 直接回主連線
—— 單庫部署（現行雲端/CI/pytest）行為與拆分前完全相同；設定後才是真雙庫。
"""
