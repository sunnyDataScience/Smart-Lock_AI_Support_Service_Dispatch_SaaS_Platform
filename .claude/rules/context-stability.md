...
3. **Tier 2** — contracts for any module/API the task touches（可用 `sunnydata-doc-freshness` skill；無該 skill 時以 frontmatter last-synced 自行檢查）
...
| 2 | AI generates + human verifies | Contract change | 直接撰寫（原 `vibecoding-*` 模板 skill 已於 2026-08-06 刪除）; carry frontmatter |
| 3 | Human curates | Quarterly | Direct edit; bump version footer |
| 4 | AI drafts on demand | Task start | 直接撰寫（原 `vibecoding-write-prd` 已刪除）; date-stamp filename |
| 5 | AI auto-regenerates | After refactor | `sunnydata-auto-regen` skill 或產生器腳本 regen; never hand-edit |
...
- Sync mechanism: `.claude/hooks/post-write.sh` + `sunnydata-doc-freshness` skill
