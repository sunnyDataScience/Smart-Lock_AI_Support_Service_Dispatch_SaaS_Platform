...
- **CR-0019 opened — 100 人技師併發壓測 CIA**（branch `docs/cr-0019-load-testing-cia`，2026-06-05）：執行 1.2.7.3.2/3/4 audit (`30847d17`) 結論「1.2.7.3.3 → 建議開 CR-0019」後續 — UAT 前 NFR 必驗項，工具引入觸發 Architecture boundary + SLA 目標需業主裁決。**取證**：(a) FastAPI uvicorn Cloud Run / smoke test `tests/smoke/api.sh` ✅ / (b) k6/Locust/Artillery 配置全空 / (c) 無 `scripts/perf/` / (d) 無 baseline metrics。**業務場景 mix**：40% pool 輪詢 / 25% accept-assign / 15% subflows / 10% reschedule / 5% complete / 5% 雜項。**5 HD 待業主裁決**：(1) 工具選型（k6/Locust/Artillery/GCP Load Testing；推薦 Locust — Python 共棲 + 分布式免費）/ (2) SLA pass/fail 門檻（p95 GET / POST / error rate / throughput / WS；推薦保守 GET 500ms / POST 1000ms / error 1% / 100 r/s / WS 99% 容 Cloud Run cold-start）/ (3) 100 VU 模擬深度（持續 30min vs Soak 4hr vs Spike vs 200 VU 50% 切換；推薦 MVP 30min）/ (4) 環境（staging Cloud Run / docker-compose 本地 / dedicated project；推薦 staging）/ (5) CI/CD 整合（manual / PR mini + pre-release full / nightly；推薦 PR mini + pre-release full）。預估 BUILD 5-7 day。status: `open-awaiting-decisions`。詳見 [`docs/_audit/CR-0019-load-testing-100-concurrent-technicians-cia.md`](docs/_audit/CR-0019-load-testing-100-concurrent-technicians-cia.md)。
