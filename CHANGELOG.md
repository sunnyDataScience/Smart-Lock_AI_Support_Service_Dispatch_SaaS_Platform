...
- **Flow 11 客戶不在場 deep audit 校正 100% → 80%**（branch `docs/flow11-agent-rewrite-audit`，2026-06-05）：執行 agent 重寫衝擊 deep audit 取證，推翻 WBS Flow 11 100% 過高 claim — 前次盤點未追 agent 端 LINE Flex template 模組刪除影響。**取證確認**：(a) backend 完整 — `propose_reschedule_v2` 寫 `saas.reschedule_proposal` ✅ / `confirm_reschedule_by_customer` / `reject_reschedule_by_customer` services + 對應 3 個 endpoints ✅；(b) web 客戶端完整 — `web/track/[token]/page.tsx` 可顯示提案、選時段、回送 confirm/reject ✅；(c) **LINE Flex 主動 push 鏈路 0%** — agent 重寫前 ReAct LangGraph node 監聽 reschedule_proposal INSERT → render Flex template → push 客戶 LINE 整段已刪（commit `0f037f45` LockCore 落地刪 200+ 檔），lockcore 無對等實作；`line_push_service` 只支援 text push（grep `flex|Flex` 0 match）；`grep -rn "reschedule_proposal\|propose_reschedule_v2\|push_reschedule" agent/` 全空。**影響**：admin propose 後客戶不會收 LINE 通知 — endpoint 形同孤兒；改期工作流退化為「admin 必須打電話通知客戶」+「客戶主動進 web/track 查看」。**不立即 BUILD 的理由**：(1) agent 重寫是 architecture lock（ADR-0109）— Flex template 重建需先決定走 lockcore tool 還是另起 LINE bot service；(2) 跨 Architecture boundary + external integration — 須先開 CIA 列 HD 取決於 lockcore 重建 vs 純 web only vs 另起獨立 LINE bot service / reschedule_proposal INSERT 觸發機制 / Flex template 歸屬 / 與 Flow 3/4/5 等其他需 LINE push 流的整合策略。**產出**：(a) `docs/_audit/flow-11-agent-rewrite-impact.md` audit doc / (b) WBS Flow 11 100% → 80% + 缺口註記補上 audit 引用 / (c) 建議下一輪開 **CR-0017 — Flow 11 LINE Flex push 重建 CIA**（業主裁決路徑）。在 CR-0017 開立並裁決前，本 audit 為 Flow 11 唯一 source of truth。
