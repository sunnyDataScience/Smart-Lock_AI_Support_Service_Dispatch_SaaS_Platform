...
- **ADR-0107 opened — LockCore supersede product_info trio**（branch `docs/adr-supersede-product-info-trio`，2026-06-05）：執行 `feat/agent-update` (commit `0f037f45`) entry 標示的「後續 ADR 工作」— 為 agent 核心重寫補 governance 層 ADR。**新 ADR-0107** `LockCore + Agent Skills 標準取代 product_info trio`（status: Accepted，date: 2026-06-05，supersedes ADR-0008/0010/0101）：Context 段引用 0f037f45 重寫範圍 / Decision 段列 lockcore + Agent Skills + LiteLLM + per-user memory + CS_TOOL_ALLOWLIST + bronze-only 規則為唯一正典 / Consequences 段標 LINE Flex push 整段刪除 (Flow 3/11/14 卡點) + Belief-Augmented ReAct 研究成果失效 + post-hoc CIA gap acknowledgment / Compliance migration 段逐項對照 ADR-0008/0010/0101 各段落在新架構處置（mega-doc → references / Turn Cycle 全失效 / dynamic lookup tool 未實作）。**三 ADR frontmatter 補 `status: superseded` + `superseded_by: ADR-0107` + `superseded_reason`**（append-only，body 不動）：ADR-0008 標明 §1/§3/§4.4 全失效；ADR-0010 標明 belief/calibrate/hypothesize/turn_cycle 全刪；ADR-0101 標明 §2.2/§2.3/§2.4 dynamic lookup / multi-tenant scope / custom SKU fallback 均未實作。**Out of Scope（後續 commit）**：(a) CLAUDE.md `Architecture Lock` 表格末尾「待補 superseded 標記」備註移除 / (b) INDEX.md 補 ADR-0107 entry。詳見 [`docs/architecture/adr/ADR-0107-lockcore-supersede-product-info-trio.md`](docs/architecture/adr/ADR-0107-lockcore-supersede-product-info-trio.md)。
