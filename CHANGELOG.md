...
- **Flow 3 範圍變更 proposal INSERT + token mint 落地 → 85%**（branch `feat/flow3-scope-change-proposal`，2026-06-05）：執行 2026-06-04 Flow 3 deep audit 揪出的 3 個 critical gap 之 #1 + #2（#3 LINE Flex 主動 push 與 Flow 11 同病，留 CR-0017）。**Gap #1 解決**：`api/services/work_order_service.py:876 record_scope_change` 改寫 — 新增從 wo 取 technician_id + estimated_price 作 `scope_changes` 表 NOT NULL 必填欄位（422 fallback 防呆若 technician_id 為 NULL，原 _SUBFLOW_FROM 理應保證非 NULL）+ `INSERT INTO scope_changes (work_order_id, technician_id, reason, original_scope JSONB, new_scope JSONB, original_price, new_price, status='pending')` 路徑落地；event payload 補 `scope_change_id` 反向指標 + 加 `actor_user_id` 鏈路（v1+v2 router 兩 caller 同步補 `actor_user_id=user.user_id`）。**Gap #2 解決**：新增 `public_token.generate_token(scope_change_id, purpose='scope_change', ttl_days=7, tenant_id)` caller — 延遲 import 避 circular + try/except 包裝（mint 失敗 non-fatal，主流不阻斷）；mint 結果僅寫 logger.info，**暫不暴露至 API response**（避免破壞 WorkOrderEnvelope response_model，等 CR-0017 LINE Flex push 重建時決定 surface 機制；admin 仍可從 scope_changes table SELECT 查詢）。**Gap #3 留待**：LINE Flex 主動 push 與 Flow 11 同病（agent 重寫刪 Flex template render module，lockcore 無對等實作），待 CR-0017 — Flow 11 LINE Flex push 重建 CIA 開立並裁決路徑後同步處理。**設計取捨**：(a) 不破壞既有 contract — `WorkOrderEnvelope` response_model 不變，scope_change_id 留 DB 與 event payload 內供 caller 後續查詢；(b) original_scope MVP 留簡化 snapshot 標記（`{"snapshot": "from_work_order", "estimated_price": ...}`），future 可從 problem_card 摘要補強；(c) state guard 仍走 _SUBFLOW_FROM `{assigned, accepted, in_progress}`；(d) token TTL 7 天對齊 spec PM-Q9=B。**驗證**：python3 ast.parse 三檔（service + v1+v2 router）OK；無 schema 變更（scope_changes table 已存在 since Schema.sql:655）。**WBS Flow 3** 65% → **85%**。**Out of Scope（CR-0017 待開）**：(a) LINE Flex 主動 push 客戶；(b) consumer-side respondScopeChangeV2 endpoint 串接 status 更新（pending→customer_approved/rejected）；(c) admin override 路徑（status='admin_override' + approved_by）；(d) scope_change_id + public_token 暴露至 API response 供 LINE Flex caller 取用。
