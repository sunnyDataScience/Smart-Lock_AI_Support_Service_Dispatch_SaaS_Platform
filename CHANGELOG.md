...
- **Flow 8 二次派工 admin 前端整合 → 100%**（branch `feat/flow8-reassign-frontend`，2026-06-05）：承接同日 backend (`d365b049`) 補完 e2e 鏈路 — work-orders/[id]/page.tsx 加 `REASSIGN_FROM` const（accepted/in_progress）+ `canAssign` derived 擴展涵蓋 `ASSIGN_FROM ∪ REASSIGN_FROM`（4 狀態都可觸發按鈕）+ `handleAssign` 內以 `REASSIGN_FROM.has(order.status)` 判斷實際 endpoint：accepted/in_progress 走 `:reassign` (body `{technician_id, reason}`，reason 必填用 reasonText.trim() 預設 fallback「強制改派（admin override）」避免 422)；inquiring/assigned 走原 `:assign` (body `{technician_id, reason_code, reason_text?}`)。既有 AssignDialog `titleReassign` UI（current_technician_id != null 顯示）自動 reused，無需新增 Modal。**設計取捨**：(a) 單一按鈕雙路徑 — admin 視角統一「指派/改派」，不必區分 endpoint 細節；(b) reason 預設 fallback 而非阻擋 submit — 緊急改派可不填理由（vs 後端 min_length=1）；(c) 重用既有 AssignDialog 而非開新 Modal — 避免 UI 複雜度與重複維護；(d) status check 走 client-side（最終 backend `_REASSIGN_FROM` 仍是 authoritative，client 只決定路徑路由）。**WBS Flow 8** 85% → **100%**（e2e 完整：admin 在 accepted/in_progress 階段可強制改派 → dispatch_logs + work_order_events 雙表 audit → WS publish → 新技師需重新 accept）。

- **Flow 8 二次派工 deep audit + reassign backend 落地 → 85%**（branch `feat/flow8-followup-dispatch`，2026-06-05）：deep audit 揪出 Flow 8 70% 略高 — `_ASSIGN_FROM={created,assigned}` 只支援 assigned 階段換手；accepted/in_progress 想換人必須 cancel-and-rebuild（破壞性，wo_id 重發、events 斷鏈、customer history 無連續性）；dispatch_log schema 有 `action='reassign'` enum 但 0 caller。**本輪 BUILD**：(1) 新狀態機常數 `_REASSIGN_FROM={assigned,accepted,in_progress}`（涵蓋 accepted/in_progress 為新能力）/ (2) 新 service `work_order_service.reassign_order(*, tenant_id, wo_id, new_technician_id, reason, actor_user_id)`：驗 _REASSIGN_FROM 409 + 取舊技師 + 防 `NO_OP_SAME_TECHNICIAN` 422 + 驗新技師同 tenant active (404 TECHNICIAN_NOT_FOUND / 409 TECHNICIAN_NOT_AVAILABLE) + UPDATE technician_id force-back to 'assigned' + service_report append `[REASSIGN] old→new: reason` + INSERT `dispatch_logs (action='reassign', technician_id=new, notes=reason)` audit + INSERT `work_order_events (event_type='reassign', payload={old/new/reason/from_status}, actor_user_id)` subflow timeline + WS publish `work_order.reassigned` / (3) 新 endpoint `POST /tenants/{tid}/work-orders/{id}:reassign` (operation_id `reassignWorkOrderV2`，`_DISPATCH_ALLOWED_ROLES` = admin/operations_manager/tenant_admin/dispatcher/customer_service + customer_service bypass 走 `audit_log_service` 留 manual_reassign_bypass 軌跡，對齊 assignWorkOrderV2 pattern)。**設計取捨**：(a) 不破壞 wo_id — 同單 reassign 保留 events / customer history / billing chain；(b) force-back to 'assigned' — 即使原 accepted/in_progress 也回退（新技師需重新 accept）；(c) reassign 寫雙表（dispatch_logs + work_order_events）— 前者對齊既有派工 audit，後者對齊 subflow timeline 觀感（admin UI 可在工單詳情看到完整事件流）。**驗證**：python3 ast.parse 兩檔 OK；無 schema 變更（dispatch_logs.action 已有 'reassign' enum）。**WBS Flow 8** 70% → **85%**。**Out of Scope（下輪續做）**：admin work-order detail page 「強制改派」按鈕 + 技師選擇器（前端 caller，OOSCope 本輪）。
