...
- **Flow 14 排班衝突 → WS publish 鏈路落地 → 85%**（branch `feat/flow14-conflict-publish`，2026-06-05）：承接同日 Flow 12-14 deep audit (`72d0df19`) 揪出的「conflict → WS publish at admin 無觸發」gap — 純後端 BUILD，不涉 LINE Flex（與 CR-0017 解耦）。**新 service helper** `_detect_schedule_conflict_and_publish(*, tenant_id, wo_id, technician_id, window_hours=2)` 在 `api/services/work_order_service.py`：(1) 取 wo.scheduled_at（NULL 則無聲返回，無排程 wo 無衝突可言）/ (2) SELECT 同 tenant 同 technician_id 其他 active wo（排除 completed/confirmed/cancelled + 排除自身）落在 ±2hr 視窗內 / (3) 若有衝突 → INSERT `work_order_events event_type='schedule_conflict'` payload 含 `conflicting_wo_ids` + `technician_id` + `window_hours` + `scheduled_at` / (4) WS publish `/realtime/dispatch-queue` channel type='schedule_conflict_detected'（admin 已訂閱 dispatch-queue 不需新前端訂閱碼）。**接於 `assign_order` UPDATE 後 best-effort 呼叫**：不阻擋 assign（衝突為「軟訊號」由 admin 決定 reassign/reschedule），DB/WS 失敗 swallow 避免影響主流。**設計取捨**：(a) ±2hr 窗口取派工普遍車程 + 服務時長合理 buffer；(b) 軟訊號不阻擋 — 業務現實常有「同棟大樓兩戶連派」合理場景；(c) 寫 events 表保 audit trail，dispatch-queue channel 取代新建 schedule-alerts channel 減少前端訂閱負擔；(d) `_publish_and_return` 仍在 helper 後跑，使 work_order.assigned event 順序在 schedule_conflict_detected 之後（admin 先收到 wo 已 assigned，再看到 conflict 警示）。**驗證**：python3 ast.parse OK；無 schema 變更（work_order_events table 已支援任意 event_type）。**WBS Flow 14** 70% → **85%**。**剩 15% 缺口**：補救流（建議時段 / 改派建議 / 升級客戶通知）涉 LINE Flex push 與 Flow 11 同病鏈路 — 等 CR-0017 LINE Flex 重建後做 outbox 接 schedule_conflict event → 自動 push admin LINE / 客戶 Flex 等；A37 衝突告警頁屬前端 admin UI follow-up。
