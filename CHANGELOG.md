...
- **Flow 8 work-orders :reassign endpoint routing E2E smoke spec**（branch `test/e2e-smoke-flow8-reassign`，2026-06-05）：完成本 session E2E catch-up 三件套（Flow 4 + Flow 9 + Flow 8）— Flow 8 二次派工 backend `d365b049` + frontend `4f186e39` 的 status-based endpoint 路徑分流邏輯（accepted/in_progress → `:reassign`；inquiring/assigned → `:assign`）需驗證。**新 `web/tests/e2e/admin/work-orders-reassign.spec.ts`** 2 個 test：(1) **accepted 狀態 → `:reassign`** — mock GET wo (status=accepted, technician_id=old) + candidates → 點改派 → 驗證 `reassignCalled=true / assignCalled=false`，body shape `{technician_id, reason}` 不含 reason_code / (2) **assigned 狀態 → `:assign`** — 同流程但 mock wo status=assigned → 驗證 `assignCalled=true / reassignCalled=false`，body shape `{technician_id, reason_code, reason_text?}` 含 reason_code。**設計取捨**：(a) 雙 endpoint 同時 mock 但其中一個應為 0 calls — 嚴格驗 status-based 分流；(b) candidates response mock 新技師李技師供 dialog 顯示；(c) body shape 不同（reassign 用 reason，assign 用 reason_code）— assertion 鎖住此差異。**完成 WBS 1.2.7.3.1 E2E catch-up 三件套**：material-requests (Flow 4) + sentiment-alerts-escalate (Flow 9) + work-orders-reassign (Flow 8) — 對應本 session 三個 100% 達成 Flow 的關鍵 e2e 鏈路驗證。
