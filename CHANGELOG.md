...
- **Flow 9 客訴升級 admin 前端按鈕 → 100%**（branch `feat/flow9-escalate-frontend`，2026-06-05）：承接同日 backend (`e4afd80b`) 補完 e2e 鏈路 — admin sentiment-alerts page 加「升級工單」按鈕（pending 狀態才顯示，紅色 TrendingUp icon + tooltip）+ `EscalateAlertModal` component（level select operations_manager/tenant_admin + reason textarea 必填 1-500 chars + char limit countdown + cancel/submit）+ POST `/tenants/{tid}/sentiment/alerts/{id}:escalate-to-work-order` caller（body `{level, reason}`）+ optimistic alert.status pending→acknowledged + success alert 顯示對應 WO id 8 字頭 + i18n zh-TW/en 同步 `escalate.*` 區段（11 keys：button/tooltip/modalTitle/modalHint/levelLabel/level.operations_manager/level.tenant_admin/reasonLabel/reasonPlaceholder/reasonOverLimit/submitButton/successAlert）。**設計取捨**：(a) 升級按鈕只在 pending 顯示 — acknowledged/resolved 不應再升（已處理中或結案）；(b) optimistic update 而非 refetch — 避免 round-trip 影響 UX；(c) success 用 window.alert 暫代 — 避免新增 toast 系統；(d) WO id slice(0,8) 顯示 — 與其他頁 WO 鏈接 pattern 對齊。**驗證**：JSON parse zh-TW + en OK。**WBS Flow 9** 90% → **100%**（Flow 9 e2e 完整：sentiment 偵測 → admin 看到告警 → 升級對應工單 → 工單 escalate_order 觸發 → audit log + service_report append 完成鏈路）。
