...
- **Flow 4 補料 e2e 收尾 → 100%**（branch `feat/flow4-supply-arrived`，2026-06-05）：承接同日 Flow 4 endpoint (`bd95838a`) + UI (`7fa67f3b`) 兩輪，本輪補 supply_arrived 收尾鏈路完成 Flow 4 100%。**新 event_type `supply_arrived`**：admin 標記某筆 material_request 已補料完成，payload 含 `material_request_event_id` 反向指標 + `note`。**新 service** `api/services/work_order_service.py mark_material_request_supplied`：驗 material_request_event 存在 + 同 tenant + 同 wo + 非 material_request → 422，DUP_SUPPLY 防重 → 409，INSERT supply_arrived event → bump wo.updated_at → WS publish `work_order.subflow.supply_arrived`。**list 過濾改寫**：`list_pending_material_requests` 加 LEFT JOIN supply_arrived events ON `(sa.payload->>'material_request_event_id')::uuid = e.id` AND `sa.id IS NULL`，自動隱藏已收尾條目。**新 endpoint** `POST /tenants/{tid}/work-orders/{id}/material-request/{eventId}:supplied` (operation_id `markMaterialRequestSuppliedV2`，admin only + cross-tenant write guard + idempotency_guard)。**前端**：material-requests page 加「標記補料完成」按鈕（CheckCircle2 icon + confirm modal + Loader2 spinner + optimistic 從 UI 移除），api.post 呼叫修正（body 為第 2 參數而非 opts.body）；i18n zh-TW/en 補 `supplyAction` / `supplyConfirm` / `col.action`。**驗證**：python3 ast.parse 兩檔語法 OK + JSON parse OK。**WBS Flow 4** 90% → **100%**（自此 Flow 4 完整 e2e：技師回報 → admin 彙整視圖 → 補料完成標記，含 dup-prevention / cross-tenant guard / WS event / actor audit trail / urgency 排序 / list 自動過濾已收尾）。

- **Flow 4 admin 補料管理彙整 UI 落地**（branch `feat/admin-material-requests-page`，2026-06-05）：承接同日上輪 endpoint（commit `bd95838a`），補前端彙整頁完成 Flow 4 admin 補料管理 e2e 鏈路。**新頁面**：`web/src/app/admin/material-requests/page.tsx`（"use client" client component）—— table view 含 urgency badge（now=紅 / today=琥珀 / tomorrow=綠 / other=灰）/ 回報時間 / 工單鏈結（`Link href=/work-orders/{id}` + ExternalLink icon，slice(0,8) 顯示）/ WO status / 排定時間 / items 摘要（`{brand} {model} ×{quantity}` join "、"）/ 備註；urgency filter chips（all / now / today / tomorrow / other 各帶計數）；refresh 按鈕；錯誤橫幅；`data-tenant` attribute。**Sidebar nav**：`web/src/components/layout/Sidebar.tsx:65` 補 `materialRequests` 入口（`/admin/material-requests`）掛在 `dispatch` 群組緊接 `dispatchQueue`。**i18n**：zh-TW / en 同步 nav label（`navigation.admin.materialRequests`）+ page 區段（`admin.materialRequests.{breadcrumb,title,count,lastUpdated,notLoaded,refresh,loading,empty,noItems,filter.*,col.*}`，JSON parse 驗證 OK）。**設計取捨**：MVP 不分 pending vs supplied（後端尚無 `supply_arrived` event_type 機制），本頁定位「近期活躍缺料事件」清單，admin 點工單 ID 進詳情頁進一步處理。**WBS Flow 4** 80% → 90%（剩 10% 為 `supply_arrived` 完結 event_type + admin 補料完成 endpoint，OOSCope 本輪）。**Out of Scope（下輪續做）**：(a) `supply_arrived` event_type 新增 + 「pending vs supplied」狀態分組；(b) admin 補料完成標記 endpoint；(c) realtime 訂閱（WS publish on material_request 事件）。
