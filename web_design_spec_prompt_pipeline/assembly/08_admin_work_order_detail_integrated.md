...
本次任務：根據上方 Global Guideline，設計並實作「工單詳情頁」。
...
- SLA 逾時使用持續性 CSS pulse 動畫（1.5s infinite），不受 Global 動畫限制
- conversation_thread 使用自訂聊天氣泡元件（非 shadcn/ui 標準元件），因需模擬 LINE 對話風格
- battery_card 使用自訂 SVG 圓環圖（非 Recharts），因為是簡單的單指標環
- 右側 sidebar 在 Desktop 斷點下使用 `position: sticky; top: 80px`，跟隨主內容捲動但保持可見
- 此頁面同時訂閱工單事件 + 設備事件 + 技師位置，可能有 3 條 WebSocket 連線，屬合理例外
