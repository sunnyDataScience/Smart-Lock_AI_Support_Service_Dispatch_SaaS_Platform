# Assembly Prompt: 進階管理 Advanced Admin
...
本次任務：根據上方 Global Guideline，設計並實作「進階管理 Advanced Admin」。
...
- 稽核日誌 JSON 詳情使用深色主題 `bg-gray-900`，與 Global 淺色 BG Page #F8FAFC 不同，此為刻意設計以提升 JSON 可讀性
- 爭議處理的證據對比面板在 Desktop 使用 `grid-cols-2` 強制並排，不受 Global 內容最大寬度限制，確保雙方證據可同時比對
- 退款 SLA 計時器使用 `setInterval` 每秒更新顯示，不受 Global 資料更新策略（polling/staleTime）管控，因為 SLA 倒數需秒級精度
- 權限矩陣在 Mobile 斷點下使用卡片式呈現，與 Global 表格 Mobile 卡片轉換規則不同（需自訂 layout 而非自動轉換），因為 12×3 矩陣無法直接轉為標準卡片
