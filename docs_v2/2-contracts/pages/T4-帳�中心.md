---
id: PAGE-T4
title: 帳戶中心
tier: 2
status: accepted
last-synced-with: pending
sync-source: code
synced-at: 2026-05-10
route: /account
version: V2.0
access_role: technician
trace_to_fr: 技師個人帳戶
related:
  - "../frontend-design-system/"
  - "../../1-decisions/module-boundary/web.md"
  - "../../5-views/_pending-merge-pages-mapping.md (legacy IA mapping)"
  - "../api/openapi.yaml (data sources)"
source_spec_path: web_design_spec_prompt_pipeline/pages/13_tech_account.md
source_spec_section: 主要
legacy_id: IA-T4
---

# T4 — 帳戶中心

## §1 Route & Auth

- **Route**: `/account`
- **Version**: V2.0
- **Access Role**: technician

## §2 Functional Requirements (Trace)

技師個人帳戶

## §3 Source Spec

完整 UI / Component / Layout / Data Source / CTA 規格見：
- 來源：`web_design_spec_prompt_pipeline/pages/13_tech_account.md`
- 區段：主要
- 對應 docs_v2 路徑：`docs_v2/extras/web-frontend/pages-legacy/13_tech_account.md` (Phase 8 後移到此)

## §4 Data Sources

TODO（待從 source spec 抽結構化 list；參見 `../api/openapi.yaml` 對應 endpoint）

## §5 Component Map

TODO（待對照 `web/src/app/account` 的 components 與 `../frontend-design-system/02-components.md`）

## §6 Change Log

| Date | Change |
| :--- | :--- |
| 2026-05-10 | thin contract created from IA mapping; full spec body待 Phase 8 SPLIT 完整化 |
