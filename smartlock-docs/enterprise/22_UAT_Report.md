...
- **範圍**：UAT-01–UAT-09 的九組 UAT 腳本（覆蓋 19 條 `SC-*` 情境）+ 14 類上線前人工驗收檢查（§3）+ KPI 門檻（§5）。
...
| **品牌小編（租戶操作方）** | 派工/工單/報價後台操作點測；可見性驗證 | 旅程 UAT-02/UAT-06 簽核 |
| **CS team（客服主管）** | AI 對話品質、接管流程、退款雙簽點測 | 旅程 UAT-01/UAT-04/UAT-06 簽核 |
| **師傅（技師代表）** | 師傅 web 接單/現場/完工流程點測 | 旅程 UAT-02/UAT-07/UAT-08 簽核 |
| **家族稽核員（Family Reviewer）** | SOP 覆核流程 + 覆核率抽驗 | 旅程 UAT-03 簽核（合約 4.4(d)）|
| **法務 / DPO** | GDPR / PII / evidence retention 驗收 | 旅程 UAT-04 簽核 |
...
## 4. UAT 情境腳本（UAT-01–UAT-09）

每段列步驟與驗收點；執行時逐項勾選並記錄異常。

> **編號沿革（2026-07-29）**：原編號 `S1`–`S9` 更名為 `UAT-01`–`UAT-09`。
> 理由是 `S<N>` 在本正典已被三種東西共用——UAT 走查、工單取消階段（`08_User_Flow §?`、
> `20_Test_Cases` TC-WO-12 的 `S1/S1.5/S2…`）、CR 實作步驟（`ADR-032`）——
> 單獨一個 `S` 不帶語義且與 `SC-*` 只差一個字母。舊討論裡的 `S3` 即今日 `UAT-03`，依此類推。

### UAT-01 消費者 LINE 自助 →（必要時）轉真人
...
### UAT-05 品牌自助配置（Agent Studio / config 治理）
...
### UAT-06 人工接管與對話不蒸發
1. 客戶完成 escalation 後，小編從派工工作台接管；持續由客戶與小編雙向發訊息。
2. 在每個 turn 前檢查 handover flag；接管期間注入旗標查詢 timeout、訊息寫入失敗與重送。
3. 解除接管後再送一則一般問題，核對 AI 僅在已解除後回覆。
- **驗收點**：接管中 AI 零插話；客戶與人工訊息皆可依 sender_role 完整追溯；故障/重送不遺漏或重複對話，且 timeout 不阻斷客戶訊息。

### UAT-07 技師註冊、KYC 到品牌授權
1. 新技師由技師 portal 註冊，填 profile、技能與服務品牌並上傳 KYC／認證文件。
2. 分別以文件不齊、未核可、已核可未授權、已核可已授權狀態嘗試進入候選池與接單。
3. 以另一品牌與 platform token 交叉檢查資料面及入口；重送註冊請求。
- **驗收點**：身分獨立於品牌租戶；僅核可且被品牌授權的技師可進正確候選池；越權一律拒絕；重送不建立重複身分或授權。

### UAT-08 認證撤銷與停權即時廣播
1. 建立同一技師在兩品牌皆可派工、且其中一筆已接單的情境。
2. 由平台撤銷認證與停權，觀察兩邊候選池、通知與進行中工單的改派；重送撤銷事件。
3. 恢復身分後檢查未經重新核可/授權前不能自行回到候選池。
- **驗收點**：撤銷後所有品牌一致排除；既有工作有可追溯人工或自動承接；事件重送不重複通知，且無任一品牌仍派給已撤銷技師。

### UAT-09 品牌申請到 License 開通上線
1. 於 landing 提交品牌申請，平台核准後執行建庫、組織／帳號建立、License 開通與 LINE sandbox 綁定。
2. 在建庫與綁定兩個中間點刻意失敗後重跑 provisioning。
3. 以新品牌、既有品牌與 platform 帳號登入／進線，檢查資料庫與 portal 邊界。
- **驗收點**：任一步未完成不得啟用 License；重跑冪等且留下 provisioning audit；新品牌僅能看到自身資料與入口，既有品牌不受影響。
...
〔標注 2026-07-11（業主裁決）：本表門檻定版——派工 SLA 達成率 ≥ 80%（參考級，記錄不擋 release）；AI 準確率＝K1 ≥ 80%（合約底線，80 題制：標準 50＋OOD 20＋對抗 10）；自助解決率＝K2 ≥ 60%（上線 3 個月後生效，UAT 期間記錄不擋）；其餘列門檻照表為準。合約紅線全集與執行清單見 `docs/uat/UAT-M1M2-acceptance-checklist.md` §1。〕
...
- **Pass**：14 類檢查全過 + UAT-01–UAT-09 全簽核 + KPI 達標 + P0/P1 清零。
...
*文件結尾 — 22_UAT_Report.md v1.1 / 2026-07-27（驗收框架版，尚無執行紀錄）*
