...
LINE 發訊 →「門鎖沒電打不開」→ AI 意圖分類 → 判定急件（被鎖門外）→ 5 分鐘內轉真人 + AI 草擬問題卡 → 小編補齊型號地址 → 開單 → 報價 Flex 推送 → 客戶 LIFF 勾選同意（金額 / 車馬費 / 保固三條款）→ 派工 → 技師到府 → 完工三硬閘（照片 ≥ 3 + 簽名 + 序號）→ 客戶確認結案評分。
...
| **現場加價 re-quote** | 師傅 App 按加價（≥ 501 元）→ 暫停施工 → 系統自動建報價 v+1（快照串鏈）→ 分層核可（501–2000 小編 / >2000 主管）→ 客戶 LIFF 確認 → 復工並更新料件；拒絕 → 按原報價完工 |
