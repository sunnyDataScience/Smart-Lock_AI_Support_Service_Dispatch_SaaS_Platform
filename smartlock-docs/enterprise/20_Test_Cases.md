...
| TC-PLT-FLOW-01 | FR-PLT-07 | 合法與非法 Flow DSL/Block fixture | 匯入合法 DSL，再匯入未知 block、缺 guard、非法狀態轉移與破壞性版本 | 非法檔在發佈前被靜態拒絕且無 runtime side effect；合法版本可審計、可回退〔標注 2026-08-05（CR-0210）：**本案屬階段二，階段一不應納入驗收範圍**。2026-08-03 靜態走查判此案「不一致」，理由是 `flow_dsl` / `block_library` / `vertical_pack` / `workflow_definition` / `積木` / `DSL` 六個識別碼在 api/web/SQL/agent/scripts 全樹零命中（以多種寫法覆核成立）。但那**不是缺陷**——`27_Product_Roadmap_WBS.md:19` 的業主裁決（2026-07-07）已把「§9 積木飛輪 / AI Onboarding Compiler / Flow 本體論」**全數劃入產品第二階段**，WBS 也已排定（`4.1 flow DSL 引擎產品化` 與 `4.4 積木庫 bootstrap` 掛在 M4 平台化地基，+8 → +20 週）；`01_MRD.md:176` 的 MR-04 狀態欄同樣標「🔜 規劃中（DSL-first 分期）」。也就是**正典自己就標了規劃中，是本 TC 拿階段二的功能去測階段一的 code**。走查已是第二輪重複判定同一件事，故就地標注避免再發生。功能本身仍是 MRD 層級的競爭定位（`01_MRD.md:133-142`「以積木本體論跨垂直」），**不建議降規格或移除**，只是驗收時點應對齊 M4。〕| failure+recovery | P1 |
