...
> 〔標注 2026-07-25（CR-0181 業主裁決）：圖中「報價過期 一般 14 天／急件 3 天」已修訂為**一般/急件統一 7 天**；客戶確認連結 confirm_token TTL 亦由 48h 改 7 天。原圖不動，實作現況以本標注為準。〕
...
> 〔標注 2026-07-25（CR-0181 業主裁決「都改七天、expire 保留」）：本表 #4「一般 14 天／急件 3 天」已修訂為**統一 7 天**，expired 單保留不清除。另 #4 後半「對話自動結案時報價同步失效」為規劃描述——code 中報價過期唯一寫入點是客戶操作時的 lazy 檢查（`quote_engine_service`），**無**對話結案連動失效之實作；7 天有效期不受 BR-CONV-01 對話 48h 結案影響。〕
