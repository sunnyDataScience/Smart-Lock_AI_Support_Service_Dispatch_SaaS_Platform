...
> **標注（2026-07-30，CR-0195 業主裁決「條件式核准」）——不改上方卡片原文**
>
> 「文件不齊 → 退件」新增一條**顯式例外**：平台管理員可在文件不齊時做**條件式核准**，
> 但必須顯式帶 `conditional` 旗標與 ≥10 字理由，且會落 `onboarding_approved_conditional`
> 生命週期事件（reason 同時記錄「當時缺什麼」與「當時的理由」）。缺省行為仍是擋下（422）。
>
> **「未過准入閘門者不得進入派工候選集」不變**——條件式核准的技師狀態就是一般 `active`，
> 本 CR 刻意不新增狀態值、不放寬任何派工資格判定。
>
> 開立本例外的原因是 as-built 與本卡片早已背離：核准端對 KYC 文件**零檢查**
> （`technician_lifecycle_service.approve_onboarding` 全鏈不查文件表），實測 6 位 active
> 技師 100% 零文件——「未驗身分即上工」一直在發生，只是無人察覺、事後也查不出誰沒補。
> 本 CR 是把這件事從隱形變成明示且可稽核，不是新開一個放行管道。
>
> **責任側仍為空白**：未完成身分驗證的技師施工之責任歸屬、保險覆蓋與揭露義務，
