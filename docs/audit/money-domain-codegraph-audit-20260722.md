...
## 裁決結果（2026-07-22，業主「照建議」）

| 項 | 裁決 | 處置 |
|---|---|---|
| 取消費階段數 | **C1：ADR-0102 為正**（6-stage） | 04_SRS 尾端標注銷差 ✅ |
| 退款「5×3」 | **D1 查證後銷差**：5×3＝5 金額 tier × SoD 三維（ADR-0040 v2 原文佐證：標題即「5 層金額分層＋SoD 三維＋Partial Refund 分類」，git `f00ac912`）；「×3」非 refund_class（5 值 enum 屬 v2 補強） | 04_SRS 尾端標注銷差 ✅ |
| 7 帳本 `ledger_type` | **E1：文件對齊實作**（傳票日記帳＋各域專表；統一單表不採） | 04_SRS 尾端標注銷差 ✅；本表 🔴 缺項就此**結案為設計取向差異**，非缺陷 |

> 同日補做：BR-SETTLE-05 對帳閘門已串入 `generate_monthly_batch`（M18 開關預設 off，
> `RECONCILE_GATE_UNMET` 409），本報告「閘門未強制串入月結」缺口銷案（merge 68f0575e）。
