...
- ✅ S-upload 兩階段 token 文件上傳 done（branch `feat/cr0115-kyc-doc-upload`，merge <待填>）：migration `090`（`technician_upload_token` 明文不落庫只存 SHA-256、48h/次數上限、`pending_approval` 才可用 + `technician_registration_document` 落師傅權威庫不入品牌庫 + `saas.technician_lifecycle_event` CHECK 追加 `kyc_reveal`）；`technician_kyc_service.py`（token 簽發/驗證/原子扣額度/magic-bytes 驗檔/寫檔+metadata）；公開端點 `POST /api/v1/technicians/registration-documents`（token-only、per-IP 限流、body 上限 middleware）；`register_technician` 回 `upload_token`（idempotency cache 存清洗副本，明文不落品牌庫）；前端註冊表單送出後接四槽位文件上傳畫面（可略過核准前補件）。
- ✅ S7 審核頁 done（同 branch）：`/platform/technicians/[id]` 加「KYC 審核資料」區塊（Tier 1 + 遮罩 PII + 文件清單）；`GET .../kyc`（遮罩）+ `POST .../kyc:reveal`（解密全值，**每次寫 lifecycle audit，fail-closed**）+ `GET .../documents/{id}`（授權讀檔、nosniff、inline 預覽）；品牌 admin 三端點 403；文件預覽走 `api.fetchBlob`（共用 401→refresh）+ blob URL cleanup。
- ✅ Review 加固 done（4 finder 多維審查 + 對抗式驗證，19 項確認全修）：idempotency token 清洗、pre-auth DoS body cap、magic-bytes 驗檔（移 HEIC）、upload_count 原子扣額、dedup 補 doc_type、寫檔失敗清孤兒、XFF 限流、reveal fail-closed、平台頁 `/kyc` allSettled、dispatch 面剔除 `/registration-documents`、compose media `external: true` + 兩 stack `KYC_ENCRYPTION_KEY`。全套 **1653 passed**（隔離 scratch DB）；tsc 0；API + 瀏覽器端到端驗證。**本機 docker（tech/platform api+web）已重建**。
- ⏳ 收尾：docs_html regen、雲端 migration 089+090、正式環境設定 `KYC_ENCRYPTION_KEY`（tech + platform 同值）— 待
