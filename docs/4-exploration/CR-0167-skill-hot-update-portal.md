...
- ✅ **rel_path 驗證閘擋掉出廠實檔修復**（2026-07-18，branch `fix/skill-relpath-unicode`）：`_REL_PATH_SEG_RE` 原為 ASCII 白名單，擋掉三個 product-knowledge 出廠實檔（Kaadas 兩個中文型號檔／3E `F(T7).md` 括號／Milre `7150+.md` 加號）；`seed_builtin_skills.py` 直 SQL INSERT 繞過驗證入庫 → 品牌後台對產品知識庫**存草稿（整樹 PUT）與發佈全 422**，HD-1 完整編輯權對該 skill 實質失效（cs-sop 純 ASCII 倖免，故 S3 驗證未暴露）。改黑名單制（禁控制字元＋`<>"|?*:`＋既有 `.`/`..`/空段/反斜線/NUL），路徑安全不依賴語系；`\n` 屬控制字元故 `\Z` 結尾繞過防護不退化。回歸測試 +2（實檔名可存可發佈／危險字元逐一仍擋），api skills 13 綠（scratch 5490）。
