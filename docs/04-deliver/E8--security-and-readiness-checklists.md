---
status: superseded
superseded_by: docs_v2/3-process/security-readiness-checklist.md
...
*   ADR 文件: `docs/01-define/adrs/adr-001 ~ adr-006`
...
*   `[x]` **數據最小化 (Data Minimization):**
    ...
    - **PII 遮罩:** AuditLogger 自動遮罩 phone, email, id_number 等個人識別資訊欄位，寫入稽核日誌前一律以 `***` 替代敏感內容 (GAP #13)。
...
*   `[x]` **功能級別授權 (Function-Level Authorization):**
    ...
    - **退款雙簽:** 金額 > 100,000 TWD 的退款操作需雙重簽核 (兩位不同 admin 角色分別核准)，防止單點授權風險 (GAP #11)。
    - **電子簽收:** 完工簽收採用 SHA-256 完整性雜湊 + 時間戳 + IP 記錄，確保簽收憑證不可竄改 (GAP #19)。
    - **RBAC 權限矩陣:** 7 角色 x resource x action 權限矩陣，所有 API 端點由 decorator 保護，角色與權限可動態調整且即時生效 (GAP #16)。
