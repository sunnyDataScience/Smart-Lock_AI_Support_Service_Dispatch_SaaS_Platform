# Assembly Prompt: 技師端 — 我的工單 My Orders
...
本次任務：根據上方 Global Guideline，設計並實作「技師端 — 我的工單 My Orders」。
...
- **completion_report_form 為全螢幕 overlay**：覆蓋整個畫面（含 BottomNav），提供沉浸式表單填寫體驗，避免誤觸導航。此設計打破 Global BottomNav 始終可見的規範。
- **signature_pad 使用 Canvas touch 事件**：需設定 `touch-action: none` 攔截頁面滑動手勢，防止簽名時頁面跟著滾動。此行為覆蓋 Global 的預設觸控捲動行為。
- **照片自動壓縮**：前端使用 `canvas.toBlob()` 壓縮至 1MB 以下再上傳，不傳原檔。此為效能優化，不受 Global 檔案上傳規範限制。
- **SLA 倒數計時**：使用前端 `setInterval` 每秒更新，不依賴 API polling 或 TanStack Query 的 refetchInterval。
- **detail_header 色條**：此頁面使用全寬色條取代標準 AppBar，不受 container padding 限制，色條延伸至螢幕邊緣。
...
- [ ] 離線模式支援（IndexedDB 草稿 + Background Sync）
