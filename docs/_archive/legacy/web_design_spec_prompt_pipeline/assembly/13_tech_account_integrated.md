# Assembly Prompt: 技師端 — 帳戶中心 Account Center
...
- **角色**：資深產品設計師與前端工程師
- **配色**：
  - Primary #2563EB (Trust Blue) / Primary Hover #1D4ED8
  - Accent #F59E0B (Amber CTA) / Accent Hover #D97706
...
- **技術棧**：Next.js 14 (App Router) + React 19 + shadcn/ui + Tailwind CSS 3.4 + TanStack Query + Zustand + Recharts + @vis.gl/react-google-maps
- **語氣**：精準、可靠、有溫度。稱呼「你」。按鈕用主動語態。錯誤先說問題再說解法。

### Tech PWA 專屬規範
- **此頁面為 Tech PWA 技師端行動應用，採 Mobile-First 設計**
- Grid：480px / 單欄 / 16px padding
- 斷點：Mobile <480px（主要設計）/ Tablet 481-768px（居中留白）/ Desktop 769px+（不支援，顯示提示頁）
- 最小觸控目標：44×44px（所有可點擊元素）
- 主要 CTA 按鈕高度 ≥ 56px，次要操作按鈕高度 ≥ 48px
- 底部安全區域：預留 safe-area-inset-bottom
...
本次任務：根據上方 Global Guideline，設計並實作「技師端 — 帳戶中心 Account Center」。
...
- **profile_header 漸層背景**：此 Section 使用品牌色漸層（#2563EB → #1E40AF）取代標準白色 BG Surface #FFFFFF 背景，打破全局背景規則，以營造技師個人品牌識別感。
- **income_overview 負 margin 疊加**：使用 margin-top: -16px 讓收入卡片與 profile_header 視覺疊加，營造層次感。此負 margin 技巧覆蓋 Global 標準間距規範。
- **sparkline 圖表**：使用輕量 SVG 繪製（或 Recharts 的 LineChart 最小配置），不引入額外圖表庫，保持 PWA bundle 輕量。若全局技術棧已含 Recharts，可直接使用其 ResponsiveContainer + LineChart。
- **week_view 緊湊佈局**：7 天 Block 寬度較小，day_label 字體 10px 低於全局最小 Caption 11px 規範，但僅用於星期單字標籤（「一」「二」等），不影響可讀性。
- **Tech PWA 不支援 Desktop**：Desktop 斷點 (769px+) 顯示「請使用手機操作」提示頁，不提供完整功能，與 Global Admin 端的全斷點支援不同。
- **availability toggle 雙向同步**：profile_header 與 settings_section 的接單開關需保持雙向同步（使用 Zustand store 統一管理），此為跨 Section 狀態共享，超出一般 Section 獨立狀態管理原則。
