...
### UI 複驗（2026-07-19 午，代理 UAT 側最終收尾）

三輪修復中「僅驗到 code/測試層」的 7 項全數 live UI 實證通過：

| 項 | 實證 |
|---|---|
| R3-7 三視圖篩選 | 群組值展開多 status 參數（network 實錄）＋期間預設真帶 7 天條件＋「全部期間」；結果與 DB 逐一一致（已派工 0=事實、已完成群組含 confirmed 語意正確） |
| R3-10 人工介入 | 候選 6/6 預設可見（原 0/7）；「可派遣」i18n；改期連結導本站工單詳情；「(A28)」已移除 |
| R3-8 KB 案例 | UI 建立成功（原 100% 404）＋詳情頁刪除（同根因順修路徑）雙實證，零殘留 |
| R3-9 匯出 | format=pdf 回 `%PDF-1.4` 真檔（application/pdf）；xlsx 422 |
| R3-4/5 通知即時 | 頁面不重整：推播後列表新增＋鈴鐺亮＋未讀 chip 三者同步（原三者皆須手動重整） |
| R3-1 apply 頁 | 申請編號＋複製鈕渲染；lookup pending 顯「審核中」＋送出時間；駁回後顯「駁回理由」全文（原永遠誤導「已受理」） |
| R3-11 排班入口 | 側欄派工管理展開含「排班申請審核」 |

測試資料（UAT0719ui/ui2）全清零。**代理 UAT 側至此完全收工**：三輪 112 findings 修復全部具備 UI 級實證。
