...
## 0. 🛑 執行前需業主定案（三件，調查查實的規格衝突，不定案 UAT 無法判分）

1. **K3 雙義衝突**：`02_BRD:322` K3＝家族覆核履約（event log 完整率 ≥95%＋dispute ≤3%；WBS gate 採此義）；但 `05_NFR:150`＋`04_SRS:514` 把 K3 對映負面情緒識別（≥90%）。**裁決：UAT 的 K3 量哪個？**（建議：兩者都驗——家族覆核與負面情緒本就各是合約條款 4.4(d)／4.4(a)，衝突只在「K3 這個編號掛誰」）
2. **K1 題數不一致**：`05_NFR` metric＝50 題、`19_Test_Plan:123` 題庫＝標準 50＋OOD 20＋對抗 10，但 `04_SRS:514` 寫「K1（200 題）」（200 題實為 K8 corpus，疑筆誤）。**裁決：K1 以 80 題制（50+20+10）判分？**（建議：是，SRS 標注勘誤）
3. **22_UAT §5 門檻表空格**：派工 SLA（參考 ≥80%）、AI 準確率／自助率欄標「[待確認：依 03_PRD 定版]」。**裁決：依本清單 §1 表定版？**
