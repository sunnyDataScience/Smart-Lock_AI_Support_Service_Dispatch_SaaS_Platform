---
status: superseded
superseded_by: docs_v2/5-views/file-dependencies.md
...
*   **本專案實踐:**
    ...
    *   **具體範例:** 當 LLM 供應商從 OpenAI GPT-4o 遷移至 Google Gemini 3 Pro（已發生，參見 `docs/01-define/adrs/adr-003-llm-integration-framework.md`），只需修改 `infrastructure/external/google_genai_client.py` 和 `infrastructure/external/langchain_chains/`，Application Layer 和 Domain Layer 完全不受影響，因為它們依賴的是穩定的 `LLMService` Protocol 抽象。
