...
```
✅ DONE:   H-Stage 0 (Foundation) + L1 core modules (decomposer, knowledge_loader, prompt, knowledge assets)
⏳ TODO:   H-Stage 1-6 (L7 → L1 wiring → L6/L3 → L2 → L5 → L8)
⛔ BLOCK:  graph/builder.py 未連接 decomposer → V1.0 P0 的最大卡點
```

| Layer | 成熟度 | V1.0 啟用 | 狀態 |
|---|---|---|---|
| L1 Task | 60% | **Yes** | Core modules done, 需 wiring |
| L2 Context | 40% | No (conditional) | Stub only |
| L3 Governance | 35% | Yes (lightweight) | Stub, config ready |
| L4 State | 45% | Already on | Existing infrastructure |
| L5 Feedback | 0% | No (conditional) | Stub only |
| L6 Safety | 15% | Yes (regex) | Stub, config ready |
| L7 Observability | 15% | **Yes (priority)** | Stub, needs backend |
| L8 Entropy | 0% | No (async only) | Stub only |
...
| WBS | 工作項目 | 產出 | 依賴 | 估時 |
|---|---|---|---|---|
| 1.1 | `@traced` decorator 實作 | `harness/observability/tracer.py` — 結構化 logging (node_name, duration_ms, status) | H-0 | 0.5d |
| 1.2 | 現有 graph nodes 加裝 decorator | `graph/nodes.py`, `agents/__init__.py` 所有 node 加 `@traced` | 1.1 | 0.5d |
| 1.3 | SessionMetrics 實作 | `harness/observability/metrics.py` — l1_hit_rate, transfer_rate, latency 追蹤 | 1.1 | 1d |
...
| WBS | 工作項目 | 產出 | 依賴 | 估時 |
|---|---|---|---|---|
| 2.1 | graph/builder.py 接入 task_decompose | 新增 node + edge: `manage_memory → task_decompose → router` | 1.5 | 1d |
| 2.2 | Router 改為 config lookup | `graph/nodes.py` router 讀 `state["task"]["intents"]`，改為 dict mapping | 2.1 | 1d |
| 2.3 | Agent prompt 注入 diagnostic_context | `agents/prompts/hardware_technician.md` 新增 `{diagnostic_context}` section | 2.1 | 0.5d |
| 2.4 | agents/__init__.py 傳遞 diagnostic_context | `build_agent_executor` 讀 `state["task"]["diagnostic_context"]` 注入 | 2.3 | 0.5d |
| 2.5 | 非技術 intent fast-path | task_decompose 偵測 non-diagnostic intent → skip Tier 2，直接輸出 intents | 2.1 | 0.5d |
| 2.6 | 整合測試 (3 canonical cases) | "鎖壞了" / "指紋沒反應" / "指紋+藍牙" → 驗證 PDCA 推理 + ProblemCard 更新 | 2.5 | 1d |
| 2.7 | Cold start 測試 | 移除所有 fault_trees/*.json → 驗證 LLM 仍可從 FM registry + component graph 推理 | 2.6 | 0.5d |
...
| WBS | 工作項目 | 產出 | 依賴 | 估時 |
|---|---|---|---|---|
| 3.1 | safety_gate 實作 | `harness/safety/gate.py` — regex keyword scan + PII check | 2.8 | 1d |
| 3.2 | safety_gate 接入 graph | edge: `task_decompose → safety_gate → router` | 3.1 | 0.5d |
| 3.3 | ToolRegistry risk levels | `harness/governance/registry.py` — 讀 config risk_levels，過濾 agent 工具集 | 2.8 | 1d |
| 3.4 | execute_tools middleware | `agents/__init__.py` 包裹 governance check (pre-call validate, post-call sanitize) | 3.3 | 1d |
| 3.5 | 整合測試 | 危險指令攔截 + 工具權限驗證 | 3.4 | 0.5d |
...
| WBS | 工作項目 | 產出 | 依賴 | 估時 |
|---|---|---|---|---|
| 4.1 | debounce 降到 2s | config `buffer_wait = 2.0` | — | 0.1d |
| 4.2 | pgvector score gate | `tools/pgvector_store.py` 加 similarity threshold 0.85 | — | 0.5d |
| 4.3 | seed data 確認 | 驗證 200+ 案例 + 手冊 PDF 已進 pgvector | — | 1d |
| 4.4 | agent prompt L3 指令 | 所有 agent prompt 加「查不到就承認不知道並轉接」| — | 0.5d |
| 4.5 | update_profile async | 不阻塞回覆，背景更新 | — | 0.5d |
| 4.6 | fallback_tools 設定 | config [[agents]] 加 fallback_tools | — | 0.5d |
...
| 7.4 | 交叉引用驗證 | Python script 驗證所有 ID 引用正確 | Backend | 0.5d |
...
| 8.3 | 案例庫 PostgreSQL | case_library 表 + 寫入邏輯 | 8.2 | 2d |
...
H-Stage 0 (DONE)
  │
  ├─ WBS 4.0 P0/P1 優化 (平行, W1-3) ──────────────────────────────┐
  │                                                                  │
  ▼                                                                  ▼
WBS 1.0 L7 Observability (W2-3)                              V1.0 non-harness ready
  │
  ▼
WBS 2.0 L1 Diagnostic Engine Wiring (W3-5)  ← CRITICAL PATH BOTTLENECK
  │
  ▼
WBS 3.0 L6/L3 Safety + Governance (W5-7)
  │
  ▼
★ V1.0 P0 Launch (W6-7) ★
  │
...
WBS 5.0 UAT (W13-15) → L7 數據分析 → V1.1 決策
  ...
  │
  ▼
WBS 7.0 Knowledge Building (W17+, 專家主導)
  │
  ▼
★ V2.0 Dispatch Launch (W18+) ★
  │
  ▼
WBS 8.0 案例庫 + 知識閉環 (W18+)
  │
  ▼
...
| Milestone | 目標週 | 交付物 | Go/No-Go 條件 |
|---|---|---|---|
| **M1: L7 上線** | W3 | trace 數據開始收集 | 5 個對話 trace 正確寫入 |
| **M2: L1 Wiring 完成** | W5 | diagnostic engine 接入 graph | 3 canonical cases PASS + cold start PASS |
| **M3: V1.0 P0 Launch** | W7 | 完整 harness 流程上線 | Zero regression in 8-intent flow |
| **M4: UAT 完成** | W15 | 50 題測試 + L7 分析報告 | ProblemCard fill rate >= 60% |
| **M5: V1.1 決策** | W16 | L2/L5 啟用/跳過決策 | L7 數據支撐 |
| **M6: Phase 1 知識資產** | W17 | 15+ fault trees + 10+ SOPs | 交叉引用驗證 PASS |
| **M7: V2.0 Launch** | W18+ | 派工 + 完工報告 + 案例庫 | 端到端流程跑通 |
| **M8: 知識閉環自轉** | W22+ | 案例庫 > 500 + 權重自動修正 | AI prediction hit rate > 70% |
