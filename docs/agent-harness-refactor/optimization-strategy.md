...
### 優化後（task_decompose 吸收分類 + Router 零 LLM）
...
task_decompose         1.5s (純分類)         2.0s (分類+SOP)
...
─────────────────────────────────────────────────────
V1.0 lite:             2.0 + 4.9 = ~7s
V2.0 full:             2.0 + 5.4 = ~7.4s
```

**V1.0: 11-20s → ~7s**。差距來自：砍 debounce 3s + async profile 1s + 合併雙 LLM 為單 LLM。
**V2.0: 原雙 LLM 設計 ~8.4s → 合併後 ~7.4s**。省去 router LLM call 1.5s，SOP 檢索 +0.5s，淨省 1s。
...
| "指紋+藍牙都不行" | hardware_tech | **Yes** | `hypothesis_formed` — 共因分析 | Tier 1 + Tier 2 (FT-HW-003 matched) |
| "怎麼換電池" | hardware_tech | **Yes** | `ready_to_conclude` — 操作指引 | Tier 1 (非故障，LLM 直接判斷) |
...
│   └── components.toml
├── knowledge/                         # JSON — 知識資產
│   ├── failures/failure_taxonomy.json
│   ├── fault_trees/FT-HW-003.json    # 每棵故障樹一個檔案
│   └── sop/SOP-HW-001.json
└── prompts/decompose_task.md          # Markdown — prompt 模板
```

```python
# 故障樹匹配: Python set 操作，不需要 SQL
matches = [
    ft for ft in knowledge_store.fault_trees
    if set(ft["required_symptoms"]) <= set(user_symptoms)
]
matches.sort(key=lambda ft: len(ft["required_symptoms"]), reverse=True)

# SOP 查找: dict lookup
sop = knowledge_store.get_sop("hardware_fault")

# 元件共享依賴判斷
def shares_dependency(comp_a, comp_b, graph):
    return bool(
        set(graph[comp_a].get("connects_to", [])) &
        set(graph[comp_b].get("connects_to", []))
    )
```

知識資產格式範例見 `knowledge/fault_trees/FT-HW-003.json` 和 `knowledge/sop/SOP-HW-001.json`。
...
| "指紋不行+藍牙不通" | `hardware_tech` | **Yes** (多症狀共現) | full mode: Yes | → hardware_technician (帶根因假設) |
| "鎖壞了+營業時間" | `hardware_tech` + `store_info` | No (鎖壞了太模糊) | full mode: Yes | → fan-out 2 agents |
...
```
lite mode (V1.0):
  Tier 1 only: LLM 意圖分類 → intents + category
  不查故障樹，不檢索 SOP，不建 TaskPlan
  延遲 ~1.5s，等效原 router

full mode (V2.0):
  Tier 1: LLM 意圖分類 → intents + category
    └─ 非診斷意圖 → 到此結束，直接派發 (延遲 ~1.5s，與 lite 相同)
    └─ 診斷意圖 + 多症狀 → 進入 Tier 2:
         Step 2: 故障樹檢索 → 根因假設 + 鑑別問題 (+0.5s)
         Step 3: SOP 檢索 → subtasks[] (+0.3s)
         Step 4: 建立/更新 ProblemCard + TaskPlan
         延遲 ~2.0-2.5s (僅技術診斷才付這個成本)
```
...
task_decompose:
  ...
  Tier 2 (診斷推理):
    Symptom: 無故障症狀，是操作問題
    confidence = 0.9 (根因明確: 用戶需要操作指引，非故障診斷)
    → 即結，不需追問
...
task_decompose:
  ...
  Tier 2 (診斷推理):
    ...
    FM 假設 (fault_trees 查詢):
      ...
      FM-ELEC-003 排線接觸不良 (30%)
      FM-POWER-002 供電波動 (20%)
    confidence = 0.35 → 需鑑別
...
故障樹權重修正 (定期批次):
  FT-HW-003 中 Samsung DP609 的歷史:
    cable_loose: 84/211 = 40% (原 30% → 上調)
    comm_board:  85/211 = 40% (原 45% → 下調)
    → 故障樹更新: cable_loose 和 comm_board 對 DP609 權重拉平

下次遇到同樣症狀 + DP609:
  "根據歷史維修紀錄，Samsung DP609 這個症狀 40% 是排線問題（維修 $800），
...
[task_decompose]
  ...
  │  │    ready_to_conclude → 給結論 + CA
  │  │    need_more_info → 注入 next_action → agent 追問
  │  ├─ Python 安全網: 最多 3 輪追問，超過建議到府
...
│  │  備註: [排線扣老化，DP609 常見問題，建議加入故障樹______]    │   │
│  │                                                          │   │
│  │  [提交審查結果]                                           │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
