...
| 元件 | 檔案 | 功能 |
|---|---|---|
| `question` 欄位 | `graph/state.py:20` | 原始使用者輸入，純文字 |
| `router` 意圖分類 | `graph/nodes.py:193-312` | LLM 分類為 8 種 intent |
| `required_slots` | `config.toml:331-333` | device_model, device_brand 兩個 slot |

### 缺口
1. **無 Goal/Subgoal 拆解**: `question` 是扁平字串，無法表達「診斷電源 → 檢查感應器 → 評估是否需派工」的多步驟結構
2. **ProblemCard 未入 code**: `moat_system_architecture.md` 定義了完整 JSON schema，但程式碼中完全不存在
3. **無 Acceptance Criteria**: 無法判斷「任務是否真正完成」
4. **無 Checkpoint**: 跨輪對話時無法恢復到上次診斷進度

### 補齊方案
- 新增 `harness/task/decomposer.py` (graph node)
- 新增 `harness/task/problem_card.py` (ProblemCard dataclass + CRUD)
- GraphState 新增 `task: dict` 欄位
...
| 散落的 `print()` | 各 nodes.py, agents/__init__.py | 非結構化 console 輸出 |
