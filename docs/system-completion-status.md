...
**最後更新：** 2026-07-09（**web 檔案層拆分——四站完全獨立專案**，branch `refactor/web-apps-split` — 業主三段裁決：檔案層真拆→共用碼複製分家（未來各站獨立改版）→web/ 收納+compose/ 集中。最終佈局 `web/{dispatch,tech,landing,platform}` 各自足專案（獨立 lockfile/Dockerfile/tests；共用碼各持副本；api.generated.ts 例外走生成腳本四份同步）；compose 五檔集中 `compose/`（.env symlink 解析）；e2e 隨站走；本 WBS 自 web/docs 遷至 docs/。ADR-028 supersede ADR-023 + CIA CR-0122。驗證：四站 npm install+tsc+build 全綠、五 compose config 過；image×4+smoke 於 merge 前補驗。）
