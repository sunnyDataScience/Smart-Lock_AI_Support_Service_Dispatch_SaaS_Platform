...
**最後更新：** 2026-07-12（**M3 D8 SSO 三站複製＋D8-a localStorage 退場（CR-0166 續，業主裁決 B）**— 承 R4/R6/R7。**D8 SSO 三站複製**：tech-portal＋platform-console 補 brand-portal 的 OIDC 授權碼流（`auth/callback` code→token→httpOnly cookie＋`auth/sso-complete` fragment→session→角色導向＋登入頁 env-gated SSO 鈕），三站 tsc 0、callback 與 brand 版位元組相同（僅 error 導向差異）、Casdoor redirect_uris 含 3000/3001/3003。**D8-a localStorage 退場（ACT-01 第一步，業主裁決 B＝可讀 claims cookie）**：三站 `api.ts` 加非 httpOnly `smartlock_claims` cookie（userId/role/tenantId/email，samesite=lax／1h，非機密）；單一寫入點 `auth.setTokens` 解 JWT 寫 cookie（SSO 與密碼登入一處覆蓋），`getCurrentSession`/`getTenantId`/`getEmail` 優先讀 cookie、缺失自動退回 localStorage（過渡零破壞）；退場 30+ 頁前端 session claims 的 localStorage 依賴，auth token 傳輸層仍 httpOnly cookie＋Authorization header（token→cookie 遷移另排）。三站邏輯位元組級一致。驗證：三站 tsc 0＋brand-portal live E2E（清空 localStorage+cookie→密碼登入→claims cookie 寫入正確→清 localStorage 後 cookie 仍在、getCurrentSession 可讀→dashboard 正常）。**M3 R0–R8＋D8＋D8-a 全落地 merge dev-ding（27+ commit）；剩 R6 待 GCP 協同、R7 端點移除待 metrics（皆治理層已定案、待外部條件）。以開發工項計 M3 已收尾。**）
