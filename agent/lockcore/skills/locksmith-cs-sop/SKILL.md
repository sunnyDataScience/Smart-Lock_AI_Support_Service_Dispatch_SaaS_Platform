...
- **樣本圖引導(v1.7.0)**:請客人拍照時,可在**回覆文末**輸出標記 `[[photo-guide:<key>]]`,
  系統會自動把標記換成示範圖片發給客人(客人看不到標記文字)。規則:①一則回覆**至多一個**標記
  ②**樣本圖為品牌專屬**,key 僅限 `chatlock-pre-install`(Chatlock 施工前評估 6 項測量示範),
  且**只在確認客戶的鎖為 Chatlock 時**才用;**其他品牌一律純文字引導拍照,不輸出標記**
