...
from lockcore.agent import context as agent_context
from lockcore.agent import model_presets as preset_helpers
from lockcore.agent.autocompact import AutoCompact
from lockcore.agent.context import ContextBuilder
from lockcore.agent.hook import AgentHook, CompositeHook
from lockcore.agent.memory import Consolidator, Dream
from lockcore.agent.progress_hook import AgentProgressHook
from lockcore.agent.runner import _MAX_INJECTIONS_PER_TURN, AgentRunner, AgentRunSpec
from lockcore.agent.subagent import SubagentManager
from lockcore.agent.tools.file_state import FileStateStore, bind_file_states, reset_file_states
from lockcore.agent.tools.message import MessageTool
from lockcore.agent.tools.registry import ToolRegistry
from lockcore.agent.tools.self import MyTool
from lockcore.bus.events import InboundMessage, OutboundMessage
from lockcore.bus.queue import MessageBus
from lockcore.command import CommandContext, CommandRouter, register_builtin_commands
from lockcore.config.schema import AgentDefaults, ModelPresetConfig
from lockcore.providers.base import LLMProvider
from lockcore.providers.factory import ProviderSnapshot
from lockcore.session.goal_state import (
    goal_state_runtime_lines,
    runner_wall_llm_timeout_s,
    sustained_goal_active,
)
from lockcore.session.manager import Session, SessionManager
from lockcore.session.webui_turns import (
    WebuiTurnCoordinator,
    build_bus_progress_callback,
    mark_webui_session,
)
from lockcore.utils.document import extract_documents
from lockcore.utils.helpers import image_placeholder_text
from lockcore.utils.helpers import truncate_text as truncate_text_fn
from lockcore.utils.image_generation_intent import image_generation_prompt
from lockcore.utils.llm_runtime import LLMRuntime
from lockcore.utils.runtime import (
    EMPTY_FINAL_RESPONSE_MESSAGE,
    SUSTAINED_GOAL_CONTINUE_PROMPT,
)

if TYPE_CHECKING:
    from lockcore.config.schema import (
        ChannelsConfig,
        ProviderConfig,
        ToolsConfig,
    )
...
class AgentLoop:
    ...
    def __init__(
        ...
        memory_manager=None,                  # [lock-cs-agent] per-user 記憶層(可選注入)
        memory_tenant: str = "locksmart",     # [lock-cs-agent] 租戶 key
        skills_dir: Path | None = None,       # [lock-cs-agent] skills 目錄(locksmith-*)
        escalation_store=None,                # [lock-cs-agent] 轉真人稽核紀錄(transfer_to_human)
        tool_allowlist: set[str] | None = None,  # [lock-cs-agent] 只保留這些工具(客服裁剪),None=不裁
        ...
        # [lock-cs-agent] per-user 記憶層注入(BUILD/SAVE 接點)
        self._memory_manager = memory_manager
        self._memory_tenant = memory_tenant
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        from lockcore.providers.factory import make_provider
    ...
    def _register_default_tools(self) -> None:
        ...
        ctx = ToolContext(
            ...
            memory_store=(self._memory_manager.provider.store if self._memory_manager else None),
            escalation_store=self._escalation_store,
            memory_tenant=self._memory_tenant,
        )
        loader = ToolLoader()
        ...
        if self._tool_allowlist is not None:
            ...
            logger.info(
                "CS tool allowlist active — kept {}, stripped {}",
                sorted(self.tools.tool_names), sorted(stripped),
            )
    ...
    async def _process_system_message(
        ...
        self._set_tool_context(
            ...
            # [lock-cs-agent] 帶當前訊息供 transfer_to_human 判定 is_explicit
            {**(msg.metadata or {}), "user_input": msg.content},
            session_key=key,
        )
    ...
    async def _state_build(self, ctx: TurnContext) -> str:
        ...
        self._set_tool_context(
            ...
            # [lock-cs-agent] 帶當前訊息供 transfer_to_human 判定 is_explicit
            {**(ctx.msg.metadata or {}), "user_input": ctx.msg.content},
