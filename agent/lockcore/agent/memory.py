...
from lockcore.agent.runner import AgentRunner, AgentRunSpec
from lockcore.agent.tools.registry import ToolRegistry
from lockcore.session.manager import Session
from lockcore.utils.gitstore import GitStore
...
if TYPE_CHECKING:
    ...
    from lockcore.session.manager import SessionManager
...
class Dream:
    ...
    def _build_tools(self) -> ToolRegistry:
        ...
        [lock-cs-agent] **拔掉自動建 skill**:不註冊 WriteFileTool —— 沒有寫檔工具,
        Dream 就無法自動產生 ``skills/<name>/SKILL.md``。多用戶客服場景禁止 agent 自我
        長 skill,以免不懂的 user「訓練」並污染共用 skill 庫(對齊「拔掉自我學習」原則)。
        只保留 read_file / edit_file 供記憶整理(編輯 MEMORY.md / USER.md)。
        """
        from lockcore.agent.tools.file_state import FileStates
        from lockcore.agent.tools.filesystem import EditFileTool, ReadFileTool
