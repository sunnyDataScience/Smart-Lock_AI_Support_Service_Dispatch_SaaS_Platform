...
from lockcore.agent.tools.schema import IntegerSchema, StringSchema, tool_parameters_schema
from lockcore.config.schema import Base
from lockcore.utils.helpers import build_image_content_blocks
...
class WebSearchConfig(Base):
    ...
    provider: str = "vertex"  # [lock-cs-agent] 預設用 Vertex Gemini grounding(Google Search)
    api_key: str = ""
    base_url: str = ""
    max_results: int = 5
    timeout: int = 30
    # [lock-cs-agent] Vertex grounding 用;project 留空則讀 VERTEX_PROJECT_ID 環境變數
    vertex_project: str = ""
    vertex_location: str = "us-central1"  # grounding 走支援區域(非主模型的 global)
...
def _validate_url_safe(url: str) -> tuple[bool, str]:
    ...
    from lockcore.security.network import validate_url_target
...
class WebSearchTool(Tool):
    ...
    name = "web_search"
    # [lock-cs-agent] 兜底工具:僅在站內知識(skill / product 文件)完全無法回答時才用。
    description = (
        "網路搜尋(兜底工具,Google Search grounded)。"
        "【使用時機】僅當你已先查過 product knowledge / 客服 skill,卻仍找不到、或文件無法回答客戶"
        "的具體問題時,才作為最後手段使用。若站內知識已能回答,**不要**用 web_search。"
        "【嚴禁用於 → 一律走 transfer_to_human】商品報價、費用、保固期限、售後條款、付款方式"
        "(網路資訊不可作為本店商業承諾依據);客戶私人資料(電話/地址/訂單)查詢。"
        "【領域外】與鎖/鑰匙/印章/汽機車/門禁/APP 無關的閒聊(美食、股票等)→ 仍按 SOP 婉拒,不要 web_search。"
        "【引用須免責】回覆若引用搜尋結果,措辭保守並加免責,如「網路資料顯示…」「依公開資訊…」,"
        "不可讓客戶誤以為是本店保證。count 預設 5(上限 10)。"
    )
    ...
    def create(cls, ctx: Any) -> Tool:
        ...
        if ctx.provider_snapshot_loader is not None:
            def config_loader():
                from lockcore.config.loader import load_config, resolve_config_env_vars
    ...
    async def execute(self, query: str, count: int | None = None, **kwargs: Any) -> str:
        ...
        if provider == "vertex":
            return await self._search_vertex(query, n)
        if provider == "olostep":
    ...
    async def _search_vertex(self, query: str, n: int) -> str:
        """[lock-cs-agent] Vertex AI Gemini grounding(Google Search)兜底搜尋。

        直接走 google.genai(Vertex)啟用內建 google_search tool 取得 grounded 回答;
        模型用 Flash(快、便宜)。回傳加免責前綴,提醒這是外部參考、非本店保證。
        """
        project = self.config.vertex_project or os.environ.get("VERTEX_PROJECT_ID", "")
        location = self.config.vertex_location or os.environ.get("VERTEX_LOCATION", "us-central1")
        if not project:
            return "(web_search 未設定:缺 vertex_project / VERTEX_PROJECT_ID)"
        ...
        def _run() -> str:
            ...
            return resp.text or "(搜尋無相關結果)"

        try:
            result = await asyncio.to_thread(_run)
        except Exception as e:  # noqa: BLE001 — 兜底工具失敗不應中斷 turn
            logger.warning(f"vertex grounding search failed: {type(e).__name__}: {e}")
            return f"(搜尋暫時無法使用:{type(e).__name__})"
