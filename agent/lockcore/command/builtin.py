...
from lockcore import __version__
from lockcore.bus.events import OutboundMessage
from lockcore.command.router import CommandContext, CommandRouter
from lockcore.utils.helpers import build_status_content
from lockcore.utils.restart import set_restart_notice_to_env
...
async def cmd_restart(ctx: CommandContext) -> OutboundMessage:
    ...
    async def _do_restart():
        ...
        os.execv(sys.executable, [sys.executable, "-m", "lockcore"] + sys.argv[1:])
...
async def cmd_status(ctx: CommandContext) -> OutboundMessage:
    ...
    with suppress(Exception):
        from lockcore.utils.searchusage import fetch_search_usage
...
async def cmd_pairing(ctx: CommandContext) -> OutboundMessage:
    """List, approve, deny or revoke pairing requests."""
    from lockcore.pairing import PAIRING_COMMAND_META_KEY, handle_pairing_command
