...
| 門打不開（門外） | **向外拉的門**→先將門「推緊」門框，解鎖後再拉開；**向內推的門**→先「拉緊」把手讓門貼合門框，解鎖後再推開（原理：消除鎖舌與受口片的摩擦力） |
| 螢幕閃爍 / 無法感應任何開鎖方式 | 鎖栓可能卡到門框受口片，導致系統偵測門未關好而拒絕操作；請先將門「推緊」或「拉緊」讓鎖栓順利對準受口片，再嘗試開鎖 |
| 指紋感應失敗 | 手指擦乾再試；若為長輩指紋較淺，**建議將同一根手指用不同角度（指尖／指腹／側邊）多登錄幾次**，或登錄紋路較清楚的不同手指；仍失敗可改用人臉、掌靜脈或密碼；多次失敗會觸發防盜鎖定需等待 |
...
| 指紋沒反應、密碼錯誤、感應不到、閃6、卡片感應不到、防盜鎖定 | 驗證失敗（指紋/密碼/卡片） | 依品牌載入：Chatlock → `load_skill("ts-verification-chatlock")`、Dormakaba → `load_skill("ts-verification-dormakaba")`、Waferlock → `load_skill("ts-verification-waferlock")`、其他 → `load_skill("ts-verification-other")` |
| 人臉辨識失敗、掌靜脈沒反應、紅燈閃、紅燈不亮、貓眼紅燈、紅燈含義、動畫在跑但沒感應、人臉不啟動 | 人臉/掌靜脈感應問題 | Chatlock → `load_skill("ts-face-chatlock")`、其他品牌 → `load_skill("ts-verification-{brand}")` |
...
| 電池很快沒電、一直沒電、鎖會漏電、沒電了、行動電源、緊急供電、Type-C、9V電池、Wi-Fi耗電、馬達變慢、換完電池仍故障 | 異常耗電/電力 | 依品牌載入：Chatlock → `load_skill("ts-power-drain-chatlock")`、Dormakaba → `load_skill("ts-power-drain-dormakaba")`、3E → `load_skill("ts-power-drain-3e")`、AiLock → `load_skill("ts-power-drain-ailock")`、其他 → `load_skill("ts-power-drain-other")` |
| 網路斷線、網路一直斷線、Wi-Fi不穩、Wi-Fi連不上、APP離線、mesh路由器、路由器設定 | 網路連線問題 | Chatlock → `load_skill("ts-wifi-chatlock")`、其他品牌 → `load_skill("ts-power-drain-{brand}")`（網路段） |
